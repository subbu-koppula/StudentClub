    ...
    <div className="min-h-screen bg-black text-white">
      <main className="container mx-auto px-4 py-12">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-400 mb-8">Chat in real-time with other students</p>
        </div>
        ...
        <Card className="mb-12 bg-white/5 border-white/10">
          <CardHeader>
            ...
            <CardDescription className="text-gray-400">
              {user ? `You are chatting as ${user.name}` : "Log in to participate in the chat"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Messages Container */}
            <div className="bg-white/5 border border-white/10 rounded-md p-4 mb-4 h-96 overflow-y-auto flex flex-col gap-4">
              {messages.length === 0 ? (
                <p className="text-gray-400 text-center my-auto">No messages yet. Start the conversation!</p>
              ) : (
                messages.map((message) => (
                  <div 
                    key={message.id}
                    className={`p-3 rounded-lg ${
                      message.username === user?.name ? "bg-blue-900/30 ml-auto" : "bg-white/10"
                    } max-w-3/4`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-gray-400">
                        {formatDistanceToNow(message.timestamp.getTime(), { addSuffix: true })}
                      </span>
                    </div>
